...
def actor(actor_num, center_model, data_queue, signal_queue, summary_queue, arg_dict):
    ...
    env = football_env.create_environment(\
                    env_name = arg_dict['env'], representation = 'raw', stacked = False, logdir = '/tmp/football',\
...
def actor_self(actor_num, center_model, data_queue, signal_queue, summary_queue, arg_dict):
    ...
    while True: # episode loop
        ...
        while not done:  # step loop
            init_t = time.time()
            is_stopped = False
            while signal_queue.qsize() > 0:                                     # 等待learner结束
                time.sleep(0.02)
                is_stopped = True
            if is_stopped:
                model.load_state_dict(center_model.state_dict())                # 更新新的学习
            wait_t += time.time() - init_t
            #
            h_in = h_out
            opp_h_in = opp_h_out
